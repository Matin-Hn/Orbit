...
async def create_update_reaction(
    ...
    try:
        # 1. Atomic increment — fast path, Redis only
        if reaction_type == ReactionType.LIKE:
            new_count = await counter.increment(video.id, current_user.id)
        else:
            # If user previously liked, switching to dislike should decrement
            new_count = await counter.decrement(video.id, current_user.id)
            if new_count is None:
                # User never liked — no counter change needed, just get current count
                new_count = await counter.get(video.id)
    ...
    try:
        ...
        try:
            if reaction_type == ReactionType.LIKE:
                await counter.decrement(video.id, current_user.id)
            else:
...
async def delete_reaction(
    ...
    try:
        new_count = await counter.get(video.id)
        # Only likes counts by redis
        if reaction_type == ReactionType.LIKE:
            # 1. Atomic decrement — fast path, Redis only
            new_count = await counter.decrement(video.id, current_user.id)
            if new_count is None:
                raise HTTPException(
    ...
    try:
        delsert_video_reaction.delay(
            reaction_type,
            current_user.id,
            video.id
        )
    except Exception as e:
        # Celery broker unreachable — compensate Redis and fail
        await counter.increment(video.id, current_user.id)
        logger.error("Celery broker unreachable during delsert_reaction", exc_info=e,
